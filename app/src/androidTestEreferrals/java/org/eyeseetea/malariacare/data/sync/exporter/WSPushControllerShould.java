...
public class WSPushControllerShould {
    ...
    public void push_callback_return_a_error_when_the_api_is_not_available()
            throws IOException {
        WSPushController mWSPushController = preferencesDependenciesRules.givenWSPushController(appInfoDataSource, deviceDataSource, mCredentialsRepository, programRepository);
    ...
    public void update_quarantine_with_correct_status_when_do_push_with_some_quarantine_surveys()
            throws IOException {
        WSPushController mWSPushController = preferencesDependenciesRules.givenWSPushController(appInfoDataSource, deviceDataSource,
                mCredentialsRepository, programRepository);
    ...
    private void whenAQuarantineResponseWithSomeQuarantineSurveysIsReceived() throws IOException {
        preferencesDependenciesRules.getMockServer().enqueueMockResponse(QUARANTINE_RESPONSE_CONFLICT);
    }

    private void whenAPushResponseWithSomeConflictsIsReceived() throws IOException{
        preferencesDependenciesRules.getMockServer().enqueueMockResponse(API_AVAILABLE_OK);
        preferencesDependenciesRules.getMockServer().enqueueMockResponse(PUSH_RESPONSE_CONFLICT);
    }
    private void whenAPushHasNotAvailableApi() throws IOException{
        preferencesDependenciesRules.getMockServer().enqueueMockResponse(API_AVAILABLE_NO_OK);
        preferencesDependenciesRules.getMockServer().enqueueMockResponse(PUSH_RESPONSE_CONFLICT);
    }
